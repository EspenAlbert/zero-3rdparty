...
def test_iter_paths_with_exclude(tmp_path):
    ...
    assert sorted(path.name for path in iter_paths(tmp_path, "*.yaml", exclude_folder_names=[".terraform"])) == [
        "1.yaml",
        "4.yaml",
    ]
...
def test_iter_paths_and_relative(tmp_path, subtests):
    ...
    with subtests.test("iter_paths_and_relative"):
        rel_paths: dict[str, Path] = {
            relative: full_path
            for full_path, relative in iter_paths_and_relative(tmp_path, "*.yaml", "*.yml", "*.txt", "*.ini")
        }
...
def test_update_between_markers_append_if_not_found(tmp_path):
    ...
    update_between_markers(path, "4th content", start_marker, end_marker, append_if_not_found=True)
    assert read_lines() == [
        "original text",
        start_marker,
        "4th content",
