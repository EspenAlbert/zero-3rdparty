...
def stem_name(path: os.PathLike, include_parent: bool = False, join_parent: str = "/") -> str:
    """
...
def clean_dir(path: Path, expected_parents: int = 2, recreate: bool = True, ignore_errors=True) -> None:
    if not running_in_container_environment():
        assert len(Path(path).parents) > expected_parents, f"rm root by accident {path}?"
    if path.exists():
    ...
    for glob in globs:
        if exclude_folder_names:
            for path in search_func(glob):
                ...
                if any(
                    f"/{folder_name}/" in rel_path or rel_path.startswith(f"{folder_name}/")
...
def read_between_markers_multiple(text: str, start_marker: str, end_marker: str) -> list[str]:
    try:
