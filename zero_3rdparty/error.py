...
class KwargsError(Exception):
    ...
    def __init__(self, **kwargs):
        as_str = ",".join(
            f"{key}={value!r}" if isinstance(key, str) else f"{key!r}={value!r}" for key, value in kwargs.items()
        )
...
class BaseError(Exception):
    ...
    def __str__(self):
        ...
        args = ", ".join(f"{key}={value!r}" for key, value in self.__dict__.items() if not key.startswith("_"))
