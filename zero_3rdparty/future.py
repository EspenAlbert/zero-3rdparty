...
def add_error_logging(future: Future, *, error_logger: logging.Logger | None = None, error_hint: str = "") -> None:
    ...
    add_done_callback(future, log_error_safely, _only_on_error=True, _include_error=True)
...
def safe_complete(
    ...
    if future.done():
        logger.warning(f"future already complete: {future}, error={error}, result={result}")
        return
    if error:
