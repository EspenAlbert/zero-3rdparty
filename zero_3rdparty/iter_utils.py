...
def public_dict(cls: Union[Type, ModuleType], recursive: bool = False) -> Dict[str, Any]:
    """
    Args:
...
@singledispatch
def select_attrs(instance: object, attrs: Iterable[str], skip_none: bool = True) -> Dict[str, object]:
    if skip_none:
        return {attr_name: attr_value for attr_name in attrs if (attr_value := getattr(instance, attr_name, None))}
    return {
...
def _select_attrs_from_dict(instance: dict, attrs: Iterable[str], skip_none: bool = True) -> Dict[str, object]:
    if skip_none:
        return {attr_name: value for attr_name in attrs if (value := instance.get(attr_name, None))}
...
def key_equal_value_to_dict(key_values: List[str]) -> Dict[str, str]:
    ...
    :return:
    """
    return dict(name_equal_value.split("=", maxsplit=1) for name_equal_value in key_values)
...
def partition(iterable: Iterable[T], pred: Callable[[T], bool] | None = None) -> Tuple[List[T], List[T]]:
...
def group_by_once(iterable: Iterable[VT], *, key: Callable[[VT], KT]) -> dict[KT, list[VT]]:
    """
    >>> example = ["a", "b", "aa", "c"]
