...
@dump_date_as_rfc3339.register
def _dump_date_as_rfc3339_none(date: None = None, strip_microseconds: bool = False) -> str:
...
def date_filename(dt: datetime | None = None) -> str:
    ...
    """
    dt = dt or utc_now()
    return dt.isoformat(sep="T", timespec="minutes").replace("+00:00", "Z").replace(":", "-")


def date_filename_with_seconds(dt: datetime | None = None, *, force_utc: bool = False) -> str:
    """
    >>> from datetime import timezone
    ...
    dt = dt or utc_now()
    if force_utc:
        dt.astimezone(UTC)  # Ensure UTC timezone
    seconds = dt.second
...
_now = utc_now()
_day_mapping = {as_day_name(dt): dt.weekday() for add_day in range(7) if (dt := _now + timedelta(days=add_day))}
