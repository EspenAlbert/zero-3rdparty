    ...
    if include_list_indexes and isinstance(container, list):
        for i, child in enumerate(container):
            ...
            if isinstance(child, dict | list):
                yield from iter_nested_keys(child, child_root_path, include_list_indexes=include_list_indexes)
        return
...
@overload
def update(container: list, simple_path: str, new_value: object, ensure_parents: bool = True) -> list: ...
...
def _as_accessor(accessor: str, start_symbols: str = "([{", end_symbols: str = ")]}") -> str | int:
    """
    >>> _as_accessor("a")
    'a'
    >>> _as_accessor("(0)")
    ...
    *must_exist, final = accessors
    ...
    for i, raw_accessor in enumerate(must_exist):
        ...
        try:
            ...
            child_container = _create_nested_container(last_container, must_exist[i + 1 :])
...
def _create_nested_container(last_container: DictList, accessors: Reversible[str]) -> DictList:
    """
