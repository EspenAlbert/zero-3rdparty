...
def func_arg_names(func: Callable, skip_self: bool = True, skip_kwargs: bool = True) -> list[str]:
...
def func_arg_types(func: Callable) -> list[type]:
    param_types = [value for _name, value in get_type_hints(func).items() if _name != "return"]
    func_name = as_name(func)
    assert len(param_types) == len(func_arg_names(func, skip_self=True, skip_kwargs=True)), (
...
def func_arg_name_of_type(func: Callable, arg_type: type, strict: bool = True) -> str | None:
...
def func_default_instances(func: Callable, default_type: type[T]) -> Iterable[tuple[str, T]]:
