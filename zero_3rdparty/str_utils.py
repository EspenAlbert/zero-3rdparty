...
def words_to_list(
    ...
    ['a', '', 'b', '', '', '', 'c']
    """
    s = s if alternative_split_char is None else s.replace(alternative_split_char, split_char)
    return [part if skip_strip else part.strip() for part in s.split(split_char) if skip_strip or part.strip()]
...
def filename_to_title(filename: str) -> str:
    ...
    words_in_filename = re.findall(r"[\w]+", filename.replace("_", " ").replace("-", " "))
    return " ".join(word.capitalize() for word in words_in_filename if re.search(r"[a-z]", word))
...
def instance_repr(instance: object, keys: Iterable[str], ignore_falsy: bool = True) -> str:
...
def skip_lines(lines: str, skip_parts: List[str]) -> str:
    ...
    """
    return "".join(
        line for line in lines.splitlines(keepends=True) if all(skip_part not in line for skip_part in skip_parts)
    )
...
def instance_repr_long_str(instance: object, *, str_len: int = 1000, replace_line_breaks: bool = True) -> str:
    """
